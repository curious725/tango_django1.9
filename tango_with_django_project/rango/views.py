...
def index(request):
    ...
    category_most_liked = Category.objects.order_by('-likes')[:5]
...
def about(request):
    return render(
        request, 'rango/about.html', context={}
    )
...
def show_category(request, category_name_slug):
    ...
    return render(
        request, 'rango/category.html', context_dict
    )
...
def add_category(request):
    ...
    return render(
        request, 'rango/add_category.html', {'form': form}
    )
...
def add_page(request, category_name_slug):
    ...
    return render(
        request, 'rango/add_page.html', context_dict
    )
...
def register(request):
    ...
    if request.method == 'POST':
        ...
        if user_form.is_valid() and profile_form.is_valid():
            ...
            # If so, we need to get if from the input form and
        ...
        # NOT HTTP POST, so we render our form
        # using two ModelForm instances.
...
def user_login(request):
    ...
    if request.method == 'POST':
        ...
        # no username with matching credentials was found.
        if user:
...
def visitor_cookie_hanlder(request):
    visits = int(get_server_side_cookie(
        request, 'visits', 1
    ))
